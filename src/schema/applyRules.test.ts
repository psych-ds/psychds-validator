...
async function setupTest(thisPath: string, fileName: string) {
  ...
  if (ddFile) {
    ...
    dsContext = new psychDSContextDataset(
      { datasetPath: thisPath } as ValidatorOptions,
      ddFile,
      description,
    );
  }
...
Deno.test({
  ...
  fn: async (t) => {
    ...
    await t.step("Columns Found", async () => {
      const { schema, context } = await setupTest(
        BASE_PATH,
      ...
      assertEquals(
        context.issues.has("CSV_COLUMN_MISSING_FROM_METADATA"),
        false,
      );
    });
    ...
    await t.step("correct sidecar identified", async () => {
      ...
      if (context.issues.has("INVALID_SCHEMAORG_PROPERTY")) {
        assert(
          context.issues.get("INVALID_SCHEMAORG_PROPERTY")?.files.has(
