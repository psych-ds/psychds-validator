  ...
  fn: async (t) => {
    ...
    if (ddFile) {
      const description = await ddFile.text()
        .then(JSON.parse);
      dsContext = new psychDSContextDataset(
        { datasetPath: PATH } as ValidatorOptions,
        ddFile,
        description,
      );
    }
    await t.step("file sidecar overwrites directory sidecar", async () => {
      ...
      if ("http://schema.org/key" in context.expandedSidecar) {
        assertEquals(context.expandedSidecar["http://schema.org/key"], [{
          "@value": "value",
        }]);
      } else {
        assertEquals(1, 2);
    ...
    await t.step(
      "directory sidecar overwrites dataset_description",
      async () => {
    ...
    // Moved all test steps into main Deno test function
    ...
    await t.step("no context in sidecar", async () => {
      ...
      if (ddFile) {
        ...
        dsContext = new psychDSContextDataset(
          { datasetPath: noCtxPATH } as ValidatorOptions,
          ddFile,
          description,
        );
      }

      const context = new psychDSContext(
        noCtxFileTree,
        file,
        issues,
        dsContext,
